...
if metals:
    cr_dir  = 'fg_2011_sh_cr' #'hm_2011_sh_final_CR/'
    ncr_dir = 'fg_2011_sh' #'hm_2011_sh_final/'
    ot_dir  = 'fg_2011_ot' #'hm_2011_nosh_metals/'
    output  = 'fg_2011_shield_combined' #'hm_2011_shield_combined/'
else:
    cr_dir  = 'fg_2011_sh_cr_mf' #'hm_2011_sh_final_mf_CR/'
    ncr_dir = 'fg_2011_sh_mf' #'hm_2011_sh_final_mf/'
    ot_dir  = 'fg_2011_ot_mf' #'hm_2011_nosh/'
    output  = 'fg_2011_shield_combined_mf' #'hm_2011_shield_combined_mf/'
...
#        if hdens[index] <= 2.0 or not metals: # normal things for low density or non-metal
        if hdens[index] <= 2.0:
            ...
            cr_data  = np.genfromtxt(cr_dir + '/' + cr_dir +'_run%i.dat'%(i), skip_header = 14, names = True)
            ncr_data = np.genfromtxt(ncr_dir + '/' + ncr_dir + '_run%i.dat'%(i), skip_header = 14, names=True)
            ot_data  = np.genfromtxt(ot_dir + '/' + ot_dir + '_run%i.dat'%(i), skip_header = 14, names = True)


            metal_crdata = np.genfromtxt(metal_crdir + '/' + metal_crdir + '_run%i.dat'%(i), skip_header = 14, names =True)
            metal_ncrdata = np.genfromtxt(metal_ncrdir + '/' + metal_ncrdir +'_run%i.dat'%(i), skip_header = 14, names=True)
            metal_otdata  = np.genfromtxt(metal_otdir + '/' + metal_otdir + '_run%i.dat'%(i), skip_header = 14, names=True)
...
#            sizes = np.array([np.size(cr_data['Te']) , np.size(ncr_data['Te']), np.size(ot_data['Te'])])
            ...
            if hdens[index] <= -8.5:
                ...
                if np.max(sizes) < 161:
                    ...
                    ot_mf = np.genfromtxt(ot_dir + '/' + ot_dir + '_run%i.dat'%(i), skip_header = 14, names=True)
...
#                output_data[name][ output_data['Te'][np.size(cr_data['Te'])] == cr_data['Te']] = cr_data[name]
        ...
        np.savetxt(output + '/' + output +'_run%i.dat'%(i), output_data, fmt='%.6E', header = "Te Heating Cooling MMW")
