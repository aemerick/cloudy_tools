...
thin   = h5py.File('/home/aemerick/code/grackle-emerick/input/CloudyData_UVB=' + DATA + '.h5')
shield = h5py.File('./CloudyData_UVB=' + DATA + '_shielded.h5')
...
# ------ load the data according to the table desired --------------------
    if data_type == 'thin': # optically thin tables
        data = h5py.File('/home/aemerick/code/grackle-emerick/input/CloudyData_UVB=' + DATA + '.h5')
    elif data_type == 'shielded': # our corrected, shielded model
        data = h5py.File('./CloudyData_UVB=' + DATA + '_shielded.h5')
    elif data_type == 'shielded_cooling_only': # an experiment, heating rates not adjusted, only cooling
        data = dd.io.load('./CloudyData_UVB=' + DATA + '_shielded_cooling_only.h5')
    elif data_type == 'noUVB': # no UV background
        data = dd.io.load('/home/aemerick/code/grackle-emerick/input/CloudyData_noUVB.h5')
...
# ----------- now do all of the plotting ---------------------   
    ...
    if data_type == 'thin':
        fig.savefig(DATA + '_cooling_rates_opticallythin.png')
    elif data_type == 'shielded':
        fig.savefig(DATA + '_cooling_rates_shielded.png')
    elif data_type == 'shielded_cooling_only':
        fig.savefig(DATA + '_cooling_rates_shielded_cooling_only.png')
    elif data_type == 'noUVB':
        fig.savefig(DATA + '_cooling_rates_noUVB.png')
...
def plot_cooling_model_comparison():
    ...
    #plot_n = [-9, -8, -6, -4 -2, -1.5, -1, -0.5, 0, 0.5, 1, 1.5, 2, 2.5, 3, 4] # densities to plot
    plot_n = [-2, -1, 0, 1, 2, 3]
...
# [(0,0),(0,1),(0,2),(0,3),(1,0),(1,1),(1,2),(1,3),(2,0),(2,1),(2,2),(2,3),(3,0),(3,1),(3,2),(3,3)]
    ...
    for i in np.arange(np.size(plot_n)): #[0,1,2,3,4,5]:
    ...
    plt.tight_layout()
    plt.subplots_adjust(wspace=0,hspace=0)
    fig.savefig(DATA + '_cooling_model_comparison')
